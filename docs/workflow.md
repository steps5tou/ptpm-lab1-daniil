...
Коммит, фиксирующий сразу множество несвязанных изменений, затрудняет поиск ошибки: непонятно, какое именно изменение сломало систему. Pull Request снижает риск регрессии тем, что изменения проверяются отдельно до попадания в main, что позволяет отклонить проблемный код ещё на этапе проверки.